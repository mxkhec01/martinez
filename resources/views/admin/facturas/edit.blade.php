...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.factura.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.facturas.update", [$factura->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('entrega') ? 'is-invalid' : '' }}" name="entrega_id" id="entrega_id" required>
                    @foreach($entregas as $id => $entry)
                        <option value="{{ $id }}" {{ (old('entrega_id') ? old('entrega_id') : $factura->entrega->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('numero_factura') ? 'is-invalid' : '' }}" type="text" name="numero_factura" id="numero_factura" value="{{ old('numero_factura', $factura->numero_factura) }}" required>
